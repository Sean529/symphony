defmodule SymphonyElixir.Config.Schema do
  ...
  defp finalize_settings(settings) do
    tracker = %{
      settings.tracker
      | api_key: resolve_optional_string_setting(settings.tracker.api_key, System.get_env("LINEAR_API_KEY")),
        project_slug:
          resolve_optional_string_setting(
  ...
  defp resolve_optional_string_setting(nil, fallback), do: normalize_optional_string_value(fallback)

  defp resolve_optional_string_setting(value, fallback) when is_binary(value) do
    case resolve_env_value(value, fallback) do
      resolved when is_binary(resolved) -> normalize_optional_string_value(resolved)
      resolved -> resolved
    end
  end
